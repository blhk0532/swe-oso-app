...
class RatsitPersonerQueueSeeder extends Seeder
{
    /**
            ...
            foreach ($chunks as $chunk) {
                $insertData = array_map(function ($record) {
                    return [
                        ...
                        'post_nummer_link' => $record['post_nummer_link'],
                        'created_at' => now(),
                        'updated_at' => now(),
                    ];
                }, $chunk);

                RatsitPersonerQueue::insert($insertData);
            }

            $this->command->info("Completed processing part {$part}");
        }

        $this->command->info('RatsitPersonerQueue seeding completed!');
    }
}

    ...
    for (const item of personItems) {
      try {
        ...
        if (phoneButton) {
          ...
          // Try to extract phone reveal ID from button data attributes
          phoneId = phoneButton.getAttribute('data-phone-id') || 
                   phoneButton.getAttribute('data-person-phone-id') ||
                   phoneButton.getAttribute('data-reveal-id') || "";
        }
        ...
        // Try to get complete phone number by visiting person's detail page
        let completeTelefon = telefon;
        if (link && telefon && telefon.trim() && !telefon.includes("+46")) {
          // Only reveal phone numbers for first 3 people per page to avoid timeout
          // This balances completeness with performance
          // Change this number to reveal more/less phone numbers
          if (results.length < 3) {
            try {
              console.log(
                `🔍 Attempting to reveal full number for ${personnamn}...`,
              );
              ...
              // Look for complete phone number in person's detail page
              // First try JSON-LD structured data
              const jsonLdScript = detailDocument.querySelector('script[type="application/ld+json"]');
              if (jsonLdScript) {
                try {
                  const jsonLdData = JSON.parse(jsonLdScript.textContent);
                  if (jsonLdData.telephone) {
                    completeTelefon = jsonLdData.telephone;
                    console.log(`✅ Found complete phone in JSON-LD: ${completeTelefon}`);
                  }
                } catch (e) {
              ...
              if (completeTelefon === telefon) {
                ...
                for (const selector of phoneSelectors) {
                  const phoneElement = detailDocument.querySelector(selector);
                  if (phoneElement) {
                    // Try href first (for tel: links)
                    let phoneText = phoneElement.getAttribute('href')?.replace('tel:', '') || "";
                    
                    // If no href, try text content
                    if (!phoneText) {
                      phoneText = phoneElement.textContent || 
                                 phoneElement.getAttribute('data-phone') || 
                                 phoneElement.getAttribute('data-telephone') || "";
                    }
                    ...
                    if (cleanPhone.length >= 10) {
                      ...
                      console.log(
                        `✅ Found complete phone in HTML (${selector}): ${completeTelefon}`,
                      );
                      break;
                    }
                  }
              ...
              // If still incomplete, mark it clearly
              if (completeTelefon === telefon) {
                console.log(`⚠️  Could not find complete number, keeping partial: ${telefon}`);
              }
              ...
              console.log(
                `❌ Error revealing number for ${personnamn}: ${revealError.message}`,
              );
            }
          } else {
            // For people beyond the reveal limit, keep the partial number
            console.log(`⏭️  Skipping phone reveal for ${personnamn} (beyond limit)`);
          }
        }
        ...
        results.push({
          ...
          karta,
          link,
          visa: "", // No longer using reveal URL approach
        });
      } catch (error) {
...
async function savePersonsViaApi(persons, searchQuery, baseUrl) {
  ...
  const endpoint = `${apiBase}/api/hitta-se/batch`;
  
  console.log(`📦 Using batch API endpoint: ${endpoint}`);
  
  // Process in batches of 50 to avoid timeout and memory issues
  const batchSize = 50;
  let totalCreated = 0;
  let totalUpdated = 0;
  let totalFailed = 0;
  ...
  for (let i = 0; i < persons.length; i += batchSize) {
    ...
    try {
      console.log(`📤 Sending batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(persons.length / batchSize)} (${batch.length} records)...`);
      ...
      if (res.status === 200 && res.data) {
        ...
        console.log(`✅ Batch complete: Created ${res.data.created}, Updated ${res.data.updated}, Failed ${res.data.failed}`);
      } else {
        totalFailed += batch.length;
        console.log(`❌ Unexpected status ${res.status} for batch`);
      }
    } catch (err) {
      totalFailed += batch.length;
      console.log(`❌ API batch failed: ${err.message}`);
      if (err.response?.data) {
        console.log(`   Error details: ${JSON.stringify(err.response.data).substring(0, 200)}`);
      }
    }

...
async function runForetagPostorterScriptForKommun(kommun) {
    try {
        console.log(
            `Running foretag postorter script for kommun: ${kommun.kommun} with URL: ${kommun.ratsit_link}`,
        );

        // Run the ratsit_foretag_postorter.mjs script with the search URL
        const { exec } = await import('child_process');
        const { promisify } = await import('util');
        const execAsync = promisify(exec);

        const { stdout, stderr } = await execAsync(
            `node ${join(projectRoot, 'scripts/ratsit_foretag_postorter.mjs')} "${kommun.ratsit_link}"`,
        ...
        console.error(
            `Error running foretag postorter script for ${kommun.kommun}:`,
            error.message,
        );
        return 0;
    }
...
async function updateKommunStatus(kommunId, status) {
    try {
        ...
        const { stdout, stderr } = await execAsync(
            `cd ${projectRoot} && php artisan tinker --execute="\\App\\Models\\RatsitForetagKommuner::where('id', ${kommunId})->update(['foretag_postort_saved' => ${status}]); echo 'Updated kommun ID ${kommunId} with foretag_postort_saved = ${status}';"`,
        );
...
async function getForetagKommunerWithLinks() {
    try {
        const { exec } = await import('child_process');
        const { promisify } = await import('util');
        const execAsync = promisify(exec);
...
\\$kommuner = \\App\\Models\\RatsitForetagKommuner::whereNotNull('ratsit_link')
    ->where('ratsit_link', '!=', '')
    ->get(['id', 'kommun', 'ratsit_link', 'foretag_postort_saved']);
...
foreach (\\$kommuner as \\$kommun) {
    echo json_encode([
        ...
        'foretag_postort_saved' => \\$kommun->foretag_postort_saved
    ]) . PHP_EOL;
}"`,
        );
...
async function main() {
    console.log('Starting Ratsit foretag postorter batch processing...');

    try {
        // Get all kommuner with ratsit_link values from the database
        const kommuner = await getForetagKommunerWithLinks();
        ...
        for (const kommun of kommuner) {
            ...
            if (kommun.foretag_postort_saved === 1) {
                console.log(
            ...
            // Run the foretag postorter script
            const status = await runForetagPostorterScriptForKommun(kommun);

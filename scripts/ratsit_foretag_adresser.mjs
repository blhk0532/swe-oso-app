...
async function saveForetagAdressData(
    postOrt,
    postNummer,
    gatuadressNamn,
    foretagCount,
    ...
    try {
        ...
        const { stdout, stderr } = await execAsync(
            `cd ${projectRoot} && php artisan tinker --execute="\\App\\Models\\RatsitForetagAdresser::create(['post_ort' => '${postOrt}', 'post_nummer' => '${postNummer}', 'gatuadress_namn' => '${gatuadressNamn}', 'foretag_count' => ${foretagCount}, 'ratsit_link' => '${ratsitLink}']); echo 'Saved foretag address: ${gatuadressNamn}, ${postNummer} ${postOrt}, Count: ${foretagCount}';"`,
        );
        ...
        console.error('Error saving foretag address data:', error.message);
        return false;
    }
}

async function scrapeRatsitForetagAdresser(url) {
    console.log(`Starting Ratsit foretag address scraping for: ${url}`);
    ...
    try {
        ...
        // Example: "Företag i Surte på postnummer 445 55 (528 st)"
        ...
        for (const ulElement of addressElements) {
            ...
            for (const li of listItems) {
                try {
                    ...
                    console.log(
                        `Found address: ${gatuadressNamn}, Count: ${foretagCount}, Link: ${fullLink}`,
                    );

                    // Save to database
                    const saved = await saveForetagAdressData(
                        postOrt,
                        postNummer,
                        gatuadressNamn,
                        foretagCount,
...
if (!url) {
    ...
    console.error(
        'Example: node ratsit_foretag_adresser.mjs "https://www.ratsit.se/foretag/Ale-kommun/Surte-44555"',
    );
    process.exit(1);
}

// Run scraper
scrapeRatsitForetagAdresser(url).catch(console.error);

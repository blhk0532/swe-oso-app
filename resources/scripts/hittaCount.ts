...
interface HittaCounts {
  hittaForetag: number;
  hittaPersoner: number;
  hittaPlatser: number;
}

async function extractHittaCounts(searchQuery: string): Promise<HittaCounts> {
  ...
  try {
    ...
    const extractCountAfterTitle = (titleText: string): number => {
...
program
  ...
  .action(async (searchQuery: string) => {
    try {
